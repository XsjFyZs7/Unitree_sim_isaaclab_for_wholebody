...
class DDSRLActionProvider(ActionProvider):
    ...
    def __init__(self,env, args_cli):
        ...
        # -- VLM Closed-Loop Control State --
        self.steps_remaining = 0
        self.current_vel_cmd = torch.tensor([0.0, 0.0, 0.0, 0.8], device=self.env.device, dtype=torch.float32)
        self.last_cmd_seq = -1
        self.execution_status = "IDLE"
        
        # Status Publisher
        try:
            self.status_pub = ChannelPublisher("rt/vlm/status", String_)
            self.status_pub.Init()
            print("[DDS] Initialized rt/vlm/status publisher")
        except Exception as e:
            print(f"[DDS] Failed to init status publisher: {e}")
    ...
    def get_vel_command(self, text):
        if "turn left" in text.lower():
            if "45" in text.lower(): return [0.0, 0.0, np.pi/2.0], 0.5
            elif "30" in text.lower(): return [0.0, 0.0, np.pi/2.0], 0.33
            elif "15" in text.lower(): return [0.0, 0.0, np.pi/2.0], 0.17
            return [0.0, 0.0, np.pi/2.0], 0.17
    ...
    def compute_current_observations(self):
        ...
        if run_command and 'run_command' in run_command:
            ...
            # Try JSON parsing (New Protocol)
            try:
                cmd_json = json.loads(run_command_data)
                if isinstance(cmd_json, dict) and "text" in cmd_json and "seq" in cmd_json:
                    seq = int(cmd_json["seq"])
                    if seq > self.last_cmd_seq:
                        text = cmd_json["text"]
                        vel, duration = self.get_vel_command(text)
                        ...
                        self.steps_remaining = int(duration / step_dt)
                        
                        # Update Command
                        new_cmd = [vel[0], vel[1], vel[2], 0.8]
                        self.current_vel_cmd = torch.tensor(new_cmd, device=self.env.device, dtype=torch.float32)
                        
                        self.last_cmd_seq = seq
                        self.execution_status = "BUSY"
                        print(f"[ActionProvider] New Command: {text} (Seq: {seq}, Duration: {duration}s, Steps: {self.steps_remaining})")
            except (json.JSONDecodeError, ValueError, TypeError):
                # Fallback to Old Protocol (Direct List String)
                try:
                    run_command_list = ast.literal_eval(run_command_data)
                    if isinstance(run_command_list, list) and len(run_command_list) >= 4:
                        # Direct command override (no step locking)
                        self.current_vel_cmd = torch.tensor(run_command_list[:4], device=self.env.device, dtype=torch.float32)
                        self.steps_remaining = 1 # Execute at least once
                except:
                    pass

        # 2. Step Execution Logic
        if self.steps_remaining > 0:
            command = self.current_vel_cmd
            self.steps_remaining -= 1
        else:
            command = torch.tensor([0.0, 0.0, 0.0, 0.8], device=self.env.device, dtype=torch.float32)
            if self.execution_status == "BUSY":
                self.execution_status = "IDLE"
                print("[ActionProvider] Action Completed. Status -> IDLE")

        # 3. Publish Status
        try:
            msg = String_(self.execution_status)
            self.status_pub.Write(msg)
        except Exception:
            pass
    ...
    def get_action(self, env) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            if self.enable_robot == "g129" and self.robot_dds:
                cmd_data = self.robot_dds.get_robot_command()
                if cmd_data and 'motor_cmd' in cmd_data:
                    print("[DEBUG] Overwriting RL action with Robot DDS command!")
                    positions = cmd_data['motor_cmd']['positions']
                    if len(positions) >= 29 and hasattr(self, "_arm_source_idx_t"):
                        self._positions_buf[:29].copy_(torch.tensor(positions[:29], dtype=torch.float32, device=self.env.device))
                        arm_vals = self._positions_buf.index_select(0, self._arm_source_idx_t)
            ...
            # print("full_action:", full_action)
            # if self.sim_step_counter % 50 == 0:
            #     print(f"[DEBUG] Step {self.sim_step_counter} | Action Mean: {full_action.mean().item():.4f} | Max: {full_action.max().item():.4f}")
            #     try:
            #         print(f"[DEBUG] Joint Pos (0-5): {self.env.scene['robot'].data.joint_pos[0, :5].cpu().numpy()}")
            #     except Exception as e:
            #         print(f"[DEBUG] Could not read joint pos: {e}")
