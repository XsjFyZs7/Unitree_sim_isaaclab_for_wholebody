...
class Measure:
    ...
    def get_robot_position(self):
        # Unitree Isaac Lab environment structure: env.scene["robot"]
...
class DistanceToGoal(Measure):
    ...
    def __init__(
        ...
        # episode data structure adaptation
        # Assuming episode is a dict with "gt_locations" (list of waypoints) or just a goal
        self._gt_waypoints: Optional[
            List[Tuple[float, float, float]]
        ] = episode.get("gt_locations", []) 
        
        # If no waypoints, use start and goal to make a simple path
        if not self._gt_waypoints and "goal_pos" in episode:
             start_pos = episode.get("start_pos", [0,0,0])
             goal_pos = episode["goal_pos"]
             self._gt_waypoints = [start_pos, goal_pos]

        if self._gt_waypoints:
...
class SPL(Measure):
    ...
    def reset_metric(self, *args: Any, **kwargs: Any):
        ...
        # Check dependency
        if DistanceToGoal.cls_uuid in self.measure_manager.measures:
            self._start_end_episode_distance = self.measure_manager.measures[
                DistanceToGoal.cls_uuid
            ].get_metric()
        else:
             # Fallback if DistanceToGoal not present (should not happen if configured correctly)
    ...
    def update_metric(
        ...
        ep_success = 0.0
        if Success.cls_uuid in self.measure_manager.measures:
        ...
        if self._start_end_episode_distance is None:
             self._metric = 0.0
        else:
...
class Success(Measure):
    ...
    def __init__(self, env, episode, measure_manager, *args: Any, **kwargs: Any):
        super().__init__(env, episode)
        # Adapt goal radius from episode data
        goals = episode.get("goals", [])
        if goals and "radius" in goals[0]:
             self._success_distance = goals[0]["radius"]
        else:
             self._success_distance = 0.5 # Default radius
    ...
    def reset_metric(self, *args: Any, **kwargs: Any):
        self.update_metric(*args, **kwargs)  # type: ignore
        # Reset is_stop_called flag on env if it exists (managed externally in main loop)
    ...
    def update_metric(self, *args: Any, **kwargs: Any):
        ...
        is_stop_called = getattr(self._env, "is_stop_called", False)
...
class OracleSuccess(Measure):
    ...
    def update_metric(self, *args: Any, **kwargs: Any):
        ...
        # OSR is 1 if at ANY point the agent was within success distance
...
def add_measurement(env, episode, measure_names=["PathLength", "DistanceToGoal", "Success", "SPL", "OracleNavigationError", "OracleSuccess"]):
    ...
    for measure_name in measure_names:
        # Dynamically instantiate classes based on name
        if measure_name in globals():
            measure_cls = globals()[measure_name]
            measure = measure_cls(env, episode, measure_manager)
            measure_manager.register_measure(measure)
        else:
            print(f"[Warning] Measure class '{measure_name}' not found.")
