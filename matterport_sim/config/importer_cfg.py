...
# [Fix] Handle missing omni.kit.asset_converter gracefully
# Define this BEFORE importing MatterportImporter to avoid circular import issues
try:
    from omni.kit.asset_converter.impl import AssetConverterContext
    _HAS_ASSET_CONVERTER = True
except ImportError:
    # Create a dummy class for type hinting and basic config storage
    class AssetConverterContext:
        def __init__(self):
            ...
            self.single_mesh = False
            self.smooth_normals = True
            self.export_preview_surface = False
            self.use_meter_as_world_unit = True
            self.create_world_as_default_root_prim = True
            self.embed_textures = True
            self.convert_fbx_to_y_up = False
            self.convert_fbx_to_z_up = True
            self.keep_all_materials = False
            self.merge_all_meshes = False
            self.use_double_precision_to_usd_transform_op = False
            self.ignore_pivots = False
            self.disabling_instancing = False
            self.export_hidden_props = False
            self.baking_scales = False
    _HAS_ASSET_CONVERTER = False

from matterport_sim.domains import MatterportImporter
