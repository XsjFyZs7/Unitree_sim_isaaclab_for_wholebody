...
class CameraPresets:
    ...
    @classmethod
    def top_down_world_camera(cls) -> CameraCfg:
        """Top-down world camera configuration based on user specification."""
        # Rotation: -90 degrees around Z-axis (Euler) -> (w, x, y, z) quaternion
        look_down_rotation = (0, 0, -1, 0)

        return CameraCfg(
            prim_path="/World/envs/env_.*/top_down_camera",
            update_period=0.02,  # Default update period
            height=1280, # Using previous resolution, not specified in new params
            width=1280, # Using previous resolution, not specified in new params
            data_types=["rgb"], # Default data type
            spawn=sim_utils.PinholeCameraCfg(
                focal_length=50.0,
                focus_distance=0.0,  # 使用默认值，因为0.0是无效值
                horizontal_aperture=266.62772,
                vertical_aperture=266.62772,
                clipping_range=(1.0, 10000000.0)
            ),
            offset=CameraCfg.OffsetCfg(
                pos=(-0.81822, 2.48195, 5.0),
                rot=look_down_rotation ,
                convention="ros"
            )
        )
        
@configclass
class CameraEnvPresets:
    """camera preset configuration collection
    ...
    def g1_front_camera(cls) -> CameraCfg:
        """front camera configuration"""
        return CameraEnvCfg.get_camera_config()
    @classmethod
    def h12_front_camera(cls) -> CameraCfg:
        """front camera configuration"""
        return CameraEnvCfg.get_camera_config(prim_path = "{ENV_REGEX_NS}/Robot/camera_link/front_cam")
    @classmethod
    def g1_world_camera(cls) -> CameraCfg:
        """front camera configuration"""
        return CameraEnvCfg.get_camera_config(prim_path="{ENV_REGEX_NS}/Robot/d435_link/PerspectiveCamera_robot",
                                                    pos_offset=(-0.9, 0.0, 0.0),
                                                    rot_offset=( -0.51292,0.51292,-0.48674, 0.48674),
                                                    focal_length = 12,
                                                    horizontal_aperture=27)
    ...
    def h12_world_camera(cls) -> CameraCfg:
        """front camera configuration"""
        return CameraEnvCfg.get_camera_config(prim_path="{ENV_REGEX_NS}/Robot/camera_link/PerspectiveCamera_robot",
                                                    pos_offset=(-0.9, 0.0, 0.0),
                                                    rot_offset=( -0.51292,0.51292,-0.48674, 0.48674),
                                                    focal_length = 12,
                                                    horizontal_aperture=27)
    ...
    def left_gripper_wrist_camera(cls) -> CameraCfg:
        """left wrist camera configuration"""
        return CameraEnvCfg.get_camera_config(
            prim_path="{ENV_REGEX_NS}/Robot/left_hand_base_link/left_wrist_camera",
            height=480,
            width=640,
            update_period=0.02,
            data_types=["rgb"],
    ...
    def right_gripper_wrist_camera(cls) -> CameraCfg:
        """right wrist camera configuration"""
        return CameraEnvCfg.get_camera_config(
            prim_path="{ENV_REGEX_NS}/Robot/right_hand_base_link/right_wrist_camera",
            height=480,
            width=640,
            update_period=0.02,
            data_types=["rgb"],
    ...
    def top_down_world_camera(cls) -> CameraCfg:
        ...
        return CameraCfg(
            prim_path="{ENV_REGEX_NS}/top_down_camera",
            update_period=0.02,  # Default update period
            height=1280, # Using previous resolution, not specified in new params
            width=1280, # Using previous resolution, not specified in new params
            data_types=["rgb"], # Default data type
